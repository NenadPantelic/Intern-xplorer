...
class AuthMiddleware:
    ...
    def _generate_401_unauthenticated(self, message="You're not authenticated to access this resource."):
        return self._generate_response(message, status.HTTP_401_UNAUTHORIZED)

    def _generate_403_unauthorized(self, message="You're not authorized to perform this action."):
        return self._generate_response(message, status.HTTP_403_FORBIDDEN)

    def _generate_response(self, message, status):
        response = Response(
            {"message": message},
